...
final fixedEndReflection1D = Video(
  category: 'waves',
  iconName: "wave",
  title: "固定端反射 (1次元)",
  videoURL: "",
  ...
  """,
  experimentWidgets: [
    const FixedEndReflection1D(),
  ],
);

class FixedEndReflection1D extends StatefulWidget with HasHeight {
  const FixedEndReflection1D({super.key});

  @override
  double get widgetHeight => 550;

  @override
  State<FixedEndReflection1D> createState() => _FixedEndReflection1DState();
}

class _FixedEndReflection1DState extends State<FixedEndReflection1D> {
  double _lambda = 4.0;
  double _periodT = 1.0;
