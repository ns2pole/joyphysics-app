...
Category _category(String name, List<Video> videos) {
  return Category(
    ...
    gifUrl: '',
    subcategories: [
      Subcategory(name: 'テスト', videos: videos),
    ],
  );
}
...
void main() {
  ...
  test('英数字は大小を無視して絞り込む', () {
    ...
    expect(
      filterArticlesByTitle(articles, 'rc').map((a) => a.video.title),
      ['RC回路'],
    );
    expect(
      filterArticlesByTitle(articles, 'Rc').map((a) => a.video.title),
  ...
  test('準備中と空コンテンツは候補から除外する', () {
    ...
    expect(
      articles.map((a) => a.video.title),
      ['自由落下', 'ウィジェットだけの落下'],
    );
    expect(
      filterArticlesByTitle(articles, '落下').map((a) => a.video.title),
      ['自由落下', 'ウィジェットだけの落下'],
    );
  });
  ...
  test('単元の並びを保ったままカテゴリ帯用にグループ化する', () {
    ...
    expect(
      groups[0].articles.map((a) => a.video.title),
      ['自由落下', '円運動'],
    );
  ...
  test('センサー記事はセンサー帯に入り、センサー名でもヒットする', () {
    ...
    expect(
      filterArticlesByTitle(articles, 'ジャイロ').map((a) => a.video.title),
      ['角速度の測定'],
    );
    expect(
      filterArticlesByTitle(articles, 'センサー').map((a) => a.video.title),
      ['角速度の測定'],
    );
    expect(
      filterArticlesByTitle(articles, '角速度').map((a) => a.video.title),
      ['角速度の測定'],
    );
  });
  ...
  test('カテゴリ名でも絞り込める', () {
    ...
    expect(
      filterArticlesByTitle(articles, '力学').map((a) => a.video.title),
      ['自由落下'],
    );
  });
